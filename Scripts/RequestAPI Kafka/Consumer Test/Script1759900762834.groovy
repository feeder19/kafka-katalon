...
def topic = GlobalVariable.topic

def bootstrap = GlobalVariable.bootstrap

def groupId = GlobalVariable.groupId

def message = CustomKeywords.'com.katalon.common.kafkaConsumerHelper.consumeMessage'(topic, bootstrap, groupId)

if (message == null) {
	
	log.logWarning("Tidak ada topic ditemukan: ${topic}")
	
} else {
	
	log.logPassed("Pesan terakhir dari Message '${topic}': ${message}")
}